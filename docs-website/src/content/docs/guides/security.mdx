...
Each Chatto deployment is a single server, so server-level roles cover everything from server-wide settings (server name, branding, blocked usernames) to room moderation.
